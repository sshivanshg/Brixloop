...
export function Navigation() {
      ...
      <nav 
        className={`mx-auto transition-all duration-500 ${
          isScrolled || isMobileMenuOpen
            ? "bg-background/80 backdrop-blur-xl border border-primary/20 rounded-2xl shadow-lg max-w-[1200px]"
            : "border-b border-primary/15 bg-transparent max-w-[1400px]"
        }`}
      >
        <div 
  ...
  <Link href="/" className="flex items-center gap-2 group">
    ...
    >
      <span className="text-foreground group-hover:text-foreground/90 transition-colors">Brix</span>
      <span className="text-primary transition-all duration-300 group-hover:drop-shadow-[0_0_14px_rgba(232,80,2,0.45)]">
        loop
      </span>
    </span>
    <span
      className={`text-primary/45 font-mono transition-all duration-500 group-hover:text-primary/70 ${isScrolled ? "text-[10px] mt-0.5" : "text-xs mt-1"}`}
    >
      TM
    </span>
  </Link>
